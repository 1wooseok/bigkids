...
시각화 도구를 통해 오늘의 뉴스를 한눈에 볼 수 있습니다.
```
...
날짜에 따른 데이터를 서버로부터 불러와 그에 맞게 키워드, 그래프, 테이블 등

화면의 주요 요소들이 <b>다시 렌더링</b> 되어야 하기 때문에,

프로젝트 초기 단순히 요청후 그래프를 그리는 함수를 호출하는 방식에 어려움을 느껴

좀 더 체계적으로 관리하기 위해 `ES6 class` 문법을 사용해 바뀌는 요소들을 컴포넌트화 하였습니다.

<i>"개발자 황준일 Vanilla Javascript로 웹 컴포넌트 만들기"<i> 를 참고하였습니다.
...
날짜를 파라미터로 요청을 하고, timeout으로 10초를 설정했습니다.
